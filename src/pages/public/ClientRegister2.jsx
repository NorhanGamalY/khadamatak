...
export default function ClientRegister2() {
    ...
    registerMutation.mutate(form, {
        ...
        onError: (err) => {
        const msg = err?.response?.data?.message || err?.response?.data?.error || "حصل خطأ أثناء التسجيل";
        setSubmitError(msg);
        },
    });
    };
    ...
    <main className="min-h-screen flex items-center justify-center">
        ...
        <div className="w-[100%] order-2 lg:order-1 md:w-[48%] flex flex-col gap-5">
            ...
            <InputField inputType="tel" title="رقم الهاتف" id="phoneNumber" fieldPlaceholder="مثال: 01012345678"
                value={form.phoneNumber} onChange={(e) => handleChange("phoneNumber", e.target.value)} error={errors.phoneNumber}
                disabled={registerMutation.isPending} />

            <div className='flex flex-col gap-1 w-full'>
                <label htmlFor="address" className='text-sm text-gray-700 font-medium'>المحافظة</label>
                <select
                id="address"
                value={form.address}
            ...
            {errors.agreeTerms && <p className="text-red-600 text-xs w-full">{errors.agreeTerms}</p>}
            {submitError && <p className="text-red-500 text-sm w-full text-center">{submitError}</p>}

            <div className="flex gap-2 w-[90%]">
                <button
                onClick={() => navigate("/client-register", { state: { fullName: form.fullName, email: form.email, password: form.password } })}
